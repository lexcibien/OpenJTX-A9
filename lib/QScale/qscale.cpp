...
double QScale::minimum() const { return m_minimum; }
...
double QScale::maximum() const { return m_maximum; }
...
double QScale::value() const { return m_value; }
...
bool QScale::isLabelsVisible() const { return m_labelsVisible; }
...
bool QScale::isScaleVisible() const { return m_scaleVisible; }
...
int QScale::borderWidth() const { return m_borderWidth; }
...
double QScale::majorStepSize() const { return m_majorStepSize; }
...
int QScale::minorStepCount() const { return m_minorStepCount; }
...
bool QScale::invertedAppearance() const { return m_invertedAppearance; }
...
Qt::Orientations QScale::orientations() const { return m_orientations; }

void QScale::resizeEvent(QResizeEvent *re) { QWidget::resizeEvent(re); }

void QScale::paintEvent(QPaintEvent * /*event*/)
{
    ...
    double hScale = NAN;
    double radius = NAN;
    double angleSpan = NAN;
    double angleStart = NAN;
    double valueSpan = NAN;
    double majorStep = NAN;
    int minorSteps = 0;
    int order = 0;
    QRectF boundingRect;
    QPointF center;
    ...
    if (!(m_orientations & Qt::Vertical) ^ !(m_orientations & Qt::Horizontal)) {
        vertical = static_cast<bool>(m_orientations & Qt::Vertical);
    } else {
    ...
    boundingRect = painter->boundingRect(QRectF(0, 0, width(), height()), Qt::AlignBottom | Qt::AlignHCenter, labelSample);
    ...
    if (radius < hScale + (.5 * hWidget) - m_borderWidth) {
        radius = (((4. * (hLabel + m_borderWidth)) + sqrt((4. * pow(hLabel + m_borderWidth, 2)) + (3. * pow(wScale, 2)))) / 3.) - hLabel
            - (2. * m_borderWidth);
    ...
    majorStep = fabs(valueSpan) * qMax(wLabel, 1.5 * boundingRect.height()) / wScale;
    order = 0;
    while (majorStep < 1) {
        majorStep *= 10, order--;
    }
    ...
    if (m_scaleVisible && majorStep != 0) {
        ...
        painter->rotate((fmod(m_minimum, majorStep / minorSteps) / valueSpan * angleSpan) - angleStart);
        int offsetCount = (minorSteps - static_cast<int>(ceil(fmod(m_minimum, majorStep) / majorStep * minorSteps))) % minorSteps;
        double scaleWidth = qMin(qMin(.25 * (hWidget - m_borderWidth), .25 * radius), 2.5 * boundingRect.height());
        for (int i = 0; i <= static_cast<int>(minorSteps * valueSpan / majorStep); i++) {
    ...
    if (m_labelsVisible && majorStep != 0) {
        for (auto i = static_cast<int>(ceil(m_minimum / majorStep)); i <= static_cast<int>(m_maximum / majorStep); i++) {
            u = pi / 180 * ((((majorStep * i) - m_minimum) / valueSpan * angleSpan) + angleStart);
            QRect position;
            Qt::Alignment align;
            if (vertical) {
                align = Qt::AlignLeft | Qt::AlignVCenter;
                position = QRect(width() - center.y() + radius * sin(u), 0, width(), height() + 2 * radius * cos(u));
            } else {
                align = Qt::AlignHCenter | Qt::AlignBottom;
                position = QRect(0, 0, 2. * (center.x() + (radius * cos(u))), center.y() - (radius * sin(u)));
            }
            painter->resetTransform();
            painter->drawText(position, align, QString::number(i * majorStep, m_labelsFormat, m_labelsPrecision));
        }
}
    ...
    polygon.setPoints(5, 0, -2, static_cast<int>(radius) - 10, -2, static_cast<int>(radius), 0, static_cast<int>(radius) - 10, 2, 0, 2);
